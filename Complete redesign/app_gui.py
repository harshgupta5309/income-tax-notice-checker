...
"""
    ...
    def _sanitize_folder_name(self, name, fallback=""):
        """Sanitizes string to be a valid directory name."""
        if not name:
            return fallback
    ...
    def _get_notice_folder_info(self, notice):
        """
        Determines the client folder, notice/demand folder, and replies folder based on notice dict.
        Rules:
        - Root: BASE_DIR (tax_backend.BASE_DIR)
        - Client folder: [PAN]
        - Outstanding Demand: {PAN}_{AY}_{DemandAmount}
        - Regular Notice / Appeal with DIN: {DIN}
        - Manual notice without DIN: MANUAL_{AY}_{Section}_{SentDate}
        """
        ...
        base_dir = tax_backend.BASE_DIR
        pan = self._sanitize_folder_name(notice.get('pan'), fallback="GENERAL_CLIENT").upper()
        ay = self._sanitize_folder_name(notice.get('ay'), fallback="AY_GENERAL")
        section = self._sanitize_folder_name(notice.get('section'), fallback="SEC_GENERAL")
        sent_date = self._sanitize_folder_name(notice.get('sent_date'), fallback="DATE_NA")
        raw_din = str(notice.get('din') or '').strip()
        din = self._sanitize_folder_name(raw_din, fallback="")
        
        # Check if this is an Outstanding Demand
        is_demand = False
        demand_amount = notice.get('demand_amount') or notice.get('amount') or ""
        proc_name = str(notice.get('proceeding_name', '')).lower()
        source_file = str(notice.get('source_file', '')).lower()
        
        if 'demand' in proc_name or 'demand' in str(section).lower() or '_ay_' in source_file or demand_amount:
            is_demand = True
            
        if is_demand:
            amt_clean = re.sub(r'[^\d.]', '', str(demand_amount)) if demand_amount else "DEMAND"
            folder_name = f"{pan}_{ay}_{amt_clean}"
        elif din and din.upper() not in ["N/A", "UNKNOWN", "NONE", ""] and not din.lower().startswith("manual_") and not din.lower().startswith("demo_"):
            folder_name = din
        else:
            folder_name = f"MANUAL_{ay}_{section}_{sent_date}"
            
        folder_name = self._sanitize_folder_name(folder_name, fallback="NOTICE_GENERAL")
        
        pan_dir = os.path.join(base_dir, pan)
        notice_dir = os.path.join(pan_dir, folder_name)
        replies_dir = os.path.join(notice_dir, "Replies")
    ...
    def ensure_notice_folder(self, notice_json):
        """Creates the notice folder and Replies subfolder, saving notice_details.json metadata."""
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            info = self._get_notice_folder_info(notice)
            os.makedirs(info["notice_dir"], exist_ok=True)
            os.makedirs(info["replies_dir"], exist_ok=True)
            
            meta_path = os.path.join(info["notice_dir"], "notice_details.json")
            ...
            return json.dumps({
                ...
                "notice_dir": info["notice_dir"],
                "replies_dir": info["replies_dir"]
            })
        except Exception as e:
            return json.dumps({"success": False, "error": str(e)})

    def get_notice_dossier(self, notice_json):
        """
        Retrieves the notice folder contents, attached notice files,
        replies folder contents (annexures/reply PDFs), and response_text.txt content.
        """
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            ...
            # Gather files directly under notice_dir (excluding Replies folder and notice_details.json)
            notice_files = []
            if os.path.exists(info["notice_dir"]):
                for fname in os.listdir(info["notice_dir"]):
                    fpath = os.path.join(info["notice_dir"], fname)
                    if os.path.isfile(fpath) and fname != "notice_details.json":
            ...
            # Gather files in Replies/ subfolder
            reply_files = []
            response_text = ""
            if os.path.exists(info["replies_dir"]):
                for fname in os.listdir(info["replies_dir"]):
                    fpath = os.path.join(info["replies_dir"], fname)
                    if os.path.isfile(fpath):
                        if fname == "response_text.txt":
                            try:
    ...
    def upload_notice_document(self, notice_json):
        """Launches native file picker to upload/attach the main notice PDF/document into notice folder."""
        import json
        import shutil
        try:
            root = tk.Tk()
            ...
            chosen_file = filedialog.askopenfilename(
                title="Select Notice Document (PDF / Scan)",
            ...
            dest_file = os.path.join(info["notice_dir"], os.path.basename(chosen_file))
            shutil.copy2(chosen_file, dest_file)
            
            notice["attachment_name"] = os.path.basename(chosen_file)
            notice["attachment_path"] = dest_file
            self.save_tracked_notice(json.dumps(notice))
    ...
    def upload_reply_document(self, notice_json):
        """Launches native file picker (multi-select) to upload reply annexures/documents into Replies/ subfolder."""
        import json
        import shutil
        try:
            root = tk.Tk()
            ...
            chosen_files = filedialog.askopenfilenames(
                title="Select Reply Annexures / Documents",
                filetypes=[
                    ("Document Files", "*.pdf;*.xlsx;*.xls;*.csv;*.docx;*.doc;*.zip;*.png;*.jpg"),
                    ("All Files", "*.*")
                ]
            )
            root.destroy()
            ...
            for f in chosen_files:
                dest_file = os.path.join(info["replies_dir"], os.path.basename(f))
    ...
    def save_reply_response_text(self, notice_json, response_text):
        """Saves what was submitted/written in the response portal into Replies/response_text.txt."""
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            info = self._get_notice_folder_info(notice)
            ...
            resp_file = os.path.join(info["replies_dir"], "response_text.txt")
            with open(resp_file, "w", encoding="utf-8") as f:
                f.write(response_text or "")
                
            # Also update notice tracker response summary
            notice["response_text_snippet"] = (response_text or "")[:150]
            self.save_tracked_notice(json.dumps(notice))
            
            return json.dumps({"success": True, "path": resp_file})
        except Exception as e:
            return json.dumps({"success": False, "error": str(e)})

    def open_notice_folder(self, notice_json):
        """Opens the specific notice folder in Windows Explorer."""
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            info = self._get_notice_folder_info(notice)
            os.makedirs(info["notice_dir"], exist_ok=True)
            if sys.platform == "win32":
                os.startfile(info["notice_dir"])
                return json.dumps({"success": True})
            else:
                import subprocess
                opener = "open" if sys.platform == "darwin" else "xdg-open"
                subprocess.call([opener, info["notice_dir"]])
                return json.dumps({"success": True})
        except Exception as e:
            return json.dumps({"success": False, "error": str(e)})

    def open_reply_folder(self, notice_json):
        """Opens the Replies/ subfolder for this notice in Windows Explorer."""
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            info = self._get_notice_folder_info(notice)
            os.makedirs(info["replies_dir"], exist_ok=True)
            if sys.platform == "win32":
                os.startfile(info["replies_dir"])
                return json.dumps({"success": True})
            else:
                import subprocess
                opener = "open" if sys.platform == "darwin" else "xdg-open"
                subprocess.call([opener, info["replies_dir"]])
                return json.dumps({"success": True})
        except Exception as e:
            return json.dumps({"success": False, "error": str(e)})

    def delete_reply_file(self, notice_json, filename):
        """Deletes a specific reply file from the Replies/ subfolder."""
        import json
        try:
            notice = json.loads(notice_json) if isinstance(notice_json, str) else notice_json
            info = self._get_notice_folder_info(notice)
            target = os.path.join(info["replies_dir"], filename)
    ...
    def sync_all_notice_folders(self):
        """Bulk creates and verifies folder structures on disk for all tracked notices."""
        import json
        try:
            tracker_path = os.path.join(tax_backend.BASE_DIR, "notice_tracker.json")
            if not os.path.exists(tracker_path):
            ...
            count = 0
            for n in notices:
                info = self._get_notice_folder_info(n)
                os.makedirs(info["notice_dir"], exist_ok=True)
                os.makedirs(info["replies_dir"], exist_ok=True)
                meta_path = os.path.join(info["notice_dir"], "notice_details.json")
                if not os.path.exists(meta_path):
                    with open(meta_path, "w", encoding="utf-8") as mf:
                        json.dump(n, mf, indent=4)
                count += 1
    ...
    def save_tracked_notice(self, notice_json):
        """Saves or updates a notice in notice_tracker.json and automatically ensures its folders exist."""
        import json
        try:
            ...
            # Ensure folders on disk
            try:
                self.ensure_notice_folder(notice)
            except Exception:
                pass
